...
const handlePaginationChange=(event, value)=>{
  ...
  //handle filters for multiple choices
  const handleFilters = (value, sectionId) => {
    const searchParams = new URLSearchParams(location.search);
    let filterValue = searchParams.getAll(sectionId);
    if (filterValue.length > 0 && filterValue[0].split(",").includes(value)) {
      filterValue = filterValue[0].split(",").filter((item) => item !== value); //removing the value from the url if again click like color=red
      if (filterValue.length == 0) {
        //if all unchecked
        searchParams.delete(sectionId); //remove value from url like ?color=
      }
    } else {
            ...
            <div className="grid grid-cols-1 gap-x-8 gap-y-10 lg:grid-cols-5">
              ...
              <div>
                <div className="flex items-center justify-between py-3">
                  <h1 className="text-lg opacity-80 font-semibold">Filters</h1>
                  <FilterListIcon />
                </div>
